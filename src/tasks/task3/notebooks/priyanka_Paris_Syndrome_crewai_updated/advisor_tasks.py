...
class AdvisorTasks():
    ...
    def gather_task(self, agent, origin, destination):
        return Task(
            ...
            Given the URL, I want you to extract the following information from the webpage:
            ...
            * List of embassies based on the user {origin}"""),
            expected_output = dedent(f"""
                    A list of the essential services like helplines, emergency contacts, embassies for the user to contact for help"""),
            agent=agent,
    ...
    def safety_advisor_task(self, agent, destination, context):
        return Task(
            description = dedent(f"""
                ...
                * Suggest places to avoid unsafe places 
                ...
                * Offer suggestions for safety procedures during traveling to avoid scams.
                """),
            expected_output = dedent(f"""
                Provide a comprehensive list of unsafe places, safety tips and guidelines 
                and also the emergency contact to reach out for help"""),
            agent = agent,
            context = [context],
